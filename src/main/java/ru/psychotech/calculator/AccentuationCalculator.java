...
public class AccentuationCalculator {
  ...
  private List<String> getRecommendations(List<ScaleWithValue> list) {
    ...
    if (list.stream()
        ...
        .filter(r -> r.getValue() >= 16)
        .count() >= 2) {
      recommendations.add("Одновременно высокие показатели демонстративности и тревожности могут указывать на попытку привлечь внимание к своему состоянию, обратиться за помощью, привлечь ресурсы окружающих.");
    }
    ...
    if (list.stream()
        ...
        .count() >= 2) {
      recommendations.add("Высокая демонстративность при низкой тревожности может означать, что человек старается убедить окружающих в своей успешности и отсутствии значимых проблем. Ярко проявляется стремление показать себя с благоприятной стороны.");
    }
    ...
    if (list.stream()
        ...
        .count() >= 2) {
      recommendations.add("Низкая демонстративность при высокой тревожности может интерпретироваться двояко. С одной стороны, это сочетание может указывать на стремление «перетерпеть», решить собственные проблемы в одиночку, своими силами.\n" +
    ...
    if (list.stream()
        ...
        .filter(r -> r.getValue() <= 8)
        .count() >= 2) {
      recommendations.add("Одновременно низкие показатели демонстративности и тревожности. При таком сочетании часто проявляется отсутствие беспокойства по поводу того, как человек будет выглядеть в глазах окружающих.");
    }
    ...
    if (list.stream()
        ...
        .count() >= 3) {
      recommendations.add("Одновременно высокие показатели возбудимости и тревожности указывают на острое состояние беспокойства, связанное с ожиданием опасного, нежелательного события. Все, что происходит вокруг человека, воспринимается как связанное с предметом беспокойства. Так, мать, тревожащаяся по поводу здоровья ребенка, воспринимает любое событие как подтверждение оправданности своего страха: ребенок слишком шумно дышит – «это одышка», ребенок дышит тихо – «да дышит ли он, не приступ ли?», ребенок закашлялся – «я так и знала!» При этом, с высокой долей вероятности, беспокойство оправдано и имеет под собой реальные основания.");
    }
    ...
    if (list.stream()
        ...
        .count() >= 2) {
      recommendations.add("Высокая возбудимость при низкой тревожности является указанием на субъективное ощущение лёгкости, открытости, безопасности ситуации.");
    }
    ...
    if (list.stream()
        ...
        .count() >= 2) {
      recommendations.add("Низкая возбудимость при высокой тревожности может быть признаком депрессивного состояния, подавленности, переживания потери, горя, с одной стороны, а также высокой мобилизованности, собранности, решимости достигнуть цели – с другой.\n" +
    ...
    if (list.stream()
        ...
        .filter(r -> r.getValue() <= 8)
        .count() >= 2) {
      recommendations.add("Одновременно низкие показатели возбудимости и тревожности могут проявиться у человека, который воспринимает ситуацию как малозначимую, не проявляет заинтересованности и беспокойства по поводу происходящих событий.");
    }
